...
camera_addr = "http://10.50.24.2:1181"

camera_width = 160
deg_per_pix = camera_width / 60
##############

tgt = ntproperty("/camera/target", (0.0, 0.0, 0.0))
...
def init(config):
    ...
    if net_cam:
        netcam_input = cv2.VideoCapture(camera_addr)
...
def run():
    ...
    if len(boxes) >= 2:
        ...
        angle = ((camera_width-1) / 2 - midx) / deg_per_pix
        tgt = (1.0, angle, 10)
        print((1.0, angle, 10))
